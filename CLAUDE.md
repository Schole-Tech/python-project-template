...
- フレームワーク: **pytest**
- 外部 API 呼び出しは **制約エミュレーション型モック**でテスト（詳細: [テストガイドライン](docs/testing-guidelines.md)）
  - モックは「成功を返す装置」ではなく「API の制約を再現する装置」として設計する
  - 入力制限（トークン上限等）・レート制限・タイムアウトをモック内で検証する
  - `side_effect` で API の実際の振る舞いを再現する
- 内部ロジックは実データでテスト
...
- 最低限のカバー対象:
  ...
  - 異常系/エッジケース 1 ケース
  - 戻り値の型・構造の検証
  - **外部依存の障害モード**（リトライ可能エラー、入力制限超過、タイムアウト）
